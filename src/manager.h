...
namespace s3d 
{
	...
	public:
		 ...
		 * Returns 0 on success or -1 on error.
		 */
		void setSource(char *id, char *source) throw (char*);
		
                /*
		 * Set the uid of the given id.
		 * Returns 0 on success or -1 on error.
		 */
		void setUID(char *id, int uid) throw (char*);
		
                /*
		 * Returns the stream of the given ID or NULL on error 
		 * (maybe id doesn't exist).
		 */
		char *get(char *name) throw (char*);

                /*
		 * Returns the stream of the given uuid or NULL on error 
		 * (maybe uuid doesn't
		 * exist).
		 */
		char *getCurrent(char *uuid) throw (char*);

                /*
		 * Returns the sourcecode of the given ID or NULL on error 
		 * (maybe id doesn't
		 * exist.
		 */
		char *getSource(char *id) throw (char*);

                /*
		 * Returns the sourcecode of the given uuid or NULL on error 
		 * (maybe uuid
		 * doesn't exist.
		 */
		char *getCurrentSource(char *name) throw (char*);
	};
};
#endif /* MANAGER_H */
